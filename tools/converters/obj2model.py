...
# Main converter
def _normalize_generated_header_path(path: str) -> str:
    if not path:
        return path
...
def convert_model_json(
    ...
    normalized_output_file = os.fspath(output_file)
    if normalized_output_file.lower().endswith((".hpp", ".hxx", ".hh")):
        normalized_output_file = os.path.splitext(normalized_output_file)[0]
    bin_out = (
        normalized_output_file
        if normalized_output_file.endswith(".bin")
        else os.path.splitext(normalized_output_file)[0] + ".bin"
    )
    ...
    # Generate C++ header
    header_out = _normalize_generated_header_path(os.path.splitext(bin_out)[0] + ".h")
